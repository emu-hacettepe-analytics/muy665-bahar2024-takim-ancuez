...
veri <- read_excel("illere göre araç sayısı.xls")
...
ggplot(en_yuksek_kisi_basi_arac, aes(x = reorder(İl, Kisi_Basina_Arac_Sayisi), y = Kisi_Basina_Arac_Sayisi)) +
  geom_bar(stat = "identity", fill = "navy") +
  geom_text(aes(label = round(Kisi_Basina_Arac_Sayisi, 2)), vjust = -0.5, color = "black") +
  labs(title = "Kişi Başına Düşen Araç Sayısının En Yüksek Olduğu 10 İl",
       x = NULL,
       y = "Kişi Başına Düşen Araç Sayısı") +
  theme_fivethirtyeight() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
